 ...
 *
 * `/instagram/content` добавлен последним: его шапка держала вес не классом, а правилом
 * `.ig-content-table th` в index.css — то есть каналом, которого статический линт не видел вовсе.
 * Именно поэтому маршрут и живёт в списке: рендер-замер ловит то, что чтение разметки пропускает.
 */
const ROUTES = [
  '/',
