...
export function MsOverview() {
  ...
  if (summary.isError) {
    ...
    if (code === 'ms_forbidden') {
      ...
      // (показатели продаж/заказов, заказы и возвраты покупателей, контрагенты, отчёты прибыльности и
      // остатков). Кнопки нет сознательно: у подключённого МойСклада на /connect нет замены токена
      ...
      return (
        <EmptyState
          ...
          reason="МойСклад не отдаёт показатели продаж и заказов сотруднику, чей токен подключён. Для дашборда ему нужен просмотр «Показателей», «Заказов покупателей», «Возвратов покупателей», «Контрагентов», отчётов «Прибыльность» и «Остатки», а также право видеть себестоимость и прибыль. Выдайте права в карточке сотрудника в МойСкладе и обновите страницу — переподключать токен не нужно."
        />
      );
    }
    if (status === 404) {
