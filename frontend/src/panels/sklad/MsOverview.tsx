...
export function MsOverview() {
  ...
  if (summary.isError) {
    ...
    if (code === 'ms_forbidden') {
      ...
      // тем же токеном ничего не изменит — называем настоящую причину.
      return (
        <EmptyState
          title="Не хватает прав в МойСкладе"
          reason="У сотрудника, чей токен подключён, нет доступа к отчётам продаж и заказов. Выдайте ему права в МойСкладе или подключите токен сотрудника с доступом."
          action={{ to: '/connect', label: 'Подключить другой токен' }}
        />
      );
    }
    if (status === 404) {
