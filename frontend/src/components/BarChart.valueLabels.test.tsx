...
describe('BarChart — плотность подписей-значений в развороте', () => {
  ...
  it('пик подписан всегда, даже если не попал в позиционный стрид', () => {
    // Прореживание позиционное и про значения ничего не знает: на разрежённом ряде оно
    // подписывало случайные единицы, а самый высокий столбец оставался немым — подписи метили
    // шум и молчали про сигнал.
    const n = 90;
    const peak = 34; // заведомо не кратен страйду
    const values = Array.from({ length: n }, (_, i) => (i === peak ? 90 : i % 7 === 0 ? 1 : 0));
    const html = render(values, monthAxis(n));
