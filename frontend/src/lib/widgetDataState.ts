...
export interface WidgetDataState {
  ...
  retry: () => void;
  /**
   * Ошибка упавшего запроса — пока только у МойСклада и Метрики (один запрос на карточку). По ней
   * тело виджета спрашивает sourceErrorKind: отзыв токена — это «Переподключить», а не «Повторить».
   */
  error?: unknown;
}
