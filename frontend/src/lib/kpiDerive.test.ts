...
/**
 * СРЕДНЕЕ ЗА ДЕНЬ (R8) — вторичное число hero-карточки «Просмотры».
 *
 * Делитель — ИЗМЕРЕННЫЕ дни архива, а не длина окна. Спека предлагала `channelViews / windowDays`,
 * но у окна «Всё» `days === 0`, и такое деление даёт Infinity; а на канале, подключённом посреди
 * окна, оно делило бы трёхдневную сумму на тридцать и печатало вдесятеро заниженное «среднее».
 * Среднее по точкам ряда — то же правило, по которому считает IG-твин (KpiHero), так что оба
 * героя продукта отвечают на «сколько за день» одной арифметикой.
 *
 * Без архива канала числа нет вовсе: headline тогда падает в пост-сумму (просмотры постов,
 * ОПУБЛИКОВАННЫХ в окне), и «в среднем за день» из него было бы враньём про природу числа.
 */
describe('deriveKpis — «в среднем за день»', () => {
  it('среднее = Σ архива ÷ ИЗМЕРЕННЫЕ дни', () => {
    const d = derive([1000, 2000, 3000, 4000], [100, 200, 300]);
    expect(d.viewsPerDay).toBe(10000 / 4);
  });

  it('окно «Всё» (days = 0) не делит на ноль', () => {
    const d = deriveKpis(
      { channel: { memberCount: 5000 }, posts: [] } as never,
      { rows: [1000, 2000].map((v, i) => ({ day: day(i), views: v, subscribers: 5000 })) } as never,
      undefined,
      null,
      0,
      null,
    ...
    expect(d.viewsPerDay).toBe(1500);
  });

  it('без архива канала среднего нет — пост-сумма не делится на дни', () => {
    expect(derive([], [100, 200, 300]).viewsPerDay).toBeNull();
  });
});
