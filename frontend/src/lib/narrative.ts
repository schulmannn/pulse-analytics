// ОТНОСИТЕЛЬНЫЙ импорт (не '@/lib/…') намеренно: движок собирается esbuild'ом в CJS-бандл для
// СЕРВЕРНОГО недельного дайджеста (frontend `npm run build:shared` → server/lib/narrative.gen.cjs),
// и алиас '@' там не резолвится. Vite относительный путь понимает одинаково — фронту без разницы.
 ...
 *  Рез — по границе слова: срез до 52, откат к последнему пробелу (если он не раньше 30-й позиции,
 *  иначе теряется полстроки), хвостовая пунктуация долой — «…18 июля присо…» не рождается. */
const clip = (s: string) => {
  if (s.length <= 52) return s;
  let cut = s.slice(0, 52);
