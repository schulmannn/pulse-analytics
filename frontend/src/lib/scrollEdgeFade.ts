...
function update(el: HTMLElement) {
  ...
  const left = overflow > EPSILON && el.scrollLeft > EPSILON ? FADE_PX : 0;
  const right = overflow > EPSILON && el.scrollLeft < overflow - EPSILON ? FADE_PX : 0;
...
export function installScrollEdgeFade(): void {
  ...
  const attach = (el: HTMLElement) => {
    // Opt-out для контейнеров, где фейд семантически лжёт (sticky-колонка пивота: она не
    // прячет контент, а маска гасила бы первые буквы её лейблов — ревью).
