...
// 2026-08-28: ДОЛГ ОТДАН, ПОТОЛКИ СРЕЗАНЫ. Поднятие «+1KB маршрутам с графиками» (2026-08-27)
//   снято, и сверх него каждый чарт-маршрут потерял ещё ~20KB. Две причины, обе — не диета кода:
//   1) В node_modules лежали ДВЕ копии всего меню-дерева Radix: react-context-menu тянул своё
//      вложенное @radix-ui/* патч-версиями новее верхнеуровневых, и обе копии ехали в сборку
//      (44.7KB raw / 15.2KB gzip дубля в чанке КАЖДОГО маршрута с графиками). Лок дедуплицирован
//      (npm update внутри тех же каретных диапазонов) — версии не менялись семантически,
//      изменилось число копий.
//   2) Редактор карточки (EditWidgetDialog) уехал за ленивую границу: он открывается по действию,
//      а статический импорт держал его вместе с ui/switch, ui/swatch-button и целым ui/select в
//      чанке каждого маршрута. Держала граница ровно одна константа — WIDGET_PERIODS, которую
//      строка пилюль тянула из модуля диалога; она переехала в chartWidget/constants.
//   ОБОЛОЧКА при этом ПОДРОСЛА на 3.1KB (JS 223.8→226.9, code 253.1→256.2) — и это не новый код:
//   после дедупа модули меню стали ОБЩИМИ у оболочки (dropdown в навигации) и у карточек
//   (context-menu), поэтому rolldown разложил одну плотную корзину 6.2KB на четыре по 2-4KB, а
//   gzip на мелких корзинах жмёт хуже. Ручная группировка в один чанк проверена и отвергнута:
//   меню тянет ui/hover-card, который живёт на публичных страницах, и именованная корзина
//   мгновенно становится eager-прелоадом бута (+29KB — ровно грабля из комментария vite.config).
//   Потолки маршрутов выставлены по факту замера + 2KB рабочего зазора.
// 2026-08-28 (вторая правка дня): −0.5…−1.9KB маршрутам метрик — сняты тексты «О метрике».
//   Блок убрали с экрана ещё в #401 («техническая информация не для конечного пользователя»),
//   но 82 записи formula/included/source остались в коде живыми свойствами объектов: tree-shaking
//   их не выбрасывает, и каждый маршрут возил прозу, которую никто не рисует. Потолки срезаны по
//   факту нового замера, зазор прежний — 2KB.
...
const BUDGETS = {
  ...
  authCode: 175 * KB,
  // 2026-08-25: +2KB всем маршрутам — студия «Оформление» (пользовательская тема). В ОБЩУЮ
  //   оболочку входит только хранилище выбора (lib/appearanceStorage, 0.5KB) и прерисовочная
  //   вставка кэша в public/theme-boot.js (0.4KB): палитры, контрастные солверы и сама студия
  //   лежат в ленивых чанках и грузятся при открытии настроек либо расхождении устройств.
  //   Замер — +1.5KB на маршрут (в т.ч. ~0.3KB общего route-CSS). Берём 2KB, потому что
  //   оболочка стояла РОВНО на потолке (220.0/220.0), а половина чарт-бюджетов — в пределах
  //   килобайта от своего.
  // 2026-08-25: +2KB всем маршрутам — левая панель студии оформления поверх приложения
  //   (её открывают, чтобы менять тему и сразу видеть графики) и выбор из реальных шрифтовых
  //   семейств. В ОБЩЕЙ оболочке лежит только флаг открытия панели, ленивая обёртка самой
  //   панели и глиф палитры в меню аккаунта; карта загрузчиков шрифтов, сами шрифты
  //   (@fontsource-variable, 15 семейств) и вся студия — в ленивых чанках, и пользователь на
  //   каноне не тянет ни байта. Замер — +1.7KB на маршрут; берём 2KB, потому что половина
  //   бюджетов после прошлой волны снова стояла в пределах килобайта от потолка.
  // 2026-08-31: +1KB маршрутам с источниками — ПЯТЫЙ источник (Rusender) в общем реестре
  //   сетей: две нав-строки за фичефлагом, бренд-глиф переключателя, иконка «конверт» и
  //   фильтр гейта в наве/палитре. Сами витрины Rusender — ленивый чанк, ни один байт их
  //   кода в эти маршруты не входит; растёт ровно оболочка реестра. Замер: +0.6KB на
  //   маршрут (moysklad 358.5→359.1), берём 1KB — после прошлой волны почти все бюджеты
  //   снова стояли в пределах полукилобайта от потолка (overview 342.8/343.0,
  //   generic 330.8/331.0, mentions 310.8/311.0).
  protectedJs: 230 * KB,
  // 2026-08-25: +2KB всем маршрутам — студия «Оформление» (пользовательская тема). В ОБЩУЮ
  //   оболочку входит только хранилище выбора (lib/appearanceStorage, 0.5KB) и прерисовочная
  //   вставка кэша в public/theme-boot.js (0.4KB): палитры, контрастные солверы и сама студия
  ...
  //   каноне не тянет ни байта. Замер — +1.7KB на маршрут; берём 2KB, потому что половина
  //   бюджетов после прошлой волны снова стояла в пределах килобайта от потолка.
  // 2026-08-25: +2KB — не рост, а ОТСУТСТВИЕ ЗАПАСА. Маршрут стоял на 339.983/340.000 — семнадцать
  //   байт, — и волна #503+#504 (морф diverging-графика в пространстве значений, потолок выбора
  //   товаров) добавила 48 байт. Гейт при этом красил не ту правку, которая виновата: соседние
  //   чарт-маршруты несут 1.1–1.7KB запаса, а этот — ноль, то есть это был не бюджет, а растяжка.
  //   Урезать код ради тридцати байт — ровно та ложная экономия, о которой сказано выше.
  // 2026-08-31: +1KB маршрутам с источниками — ПЯТЫЙ источник (Rusender) в общем реестре
  //   сетей: две нав-строки за фичефлагом, бренд-глиф переключателя, иконка «конверт» и
  //   фильтр гейта в наве/палитре. Сами витрины Rusender — ленивый чанк, ни один байт их
  //   кода в эти маршруты не входит; растёт ровно оболочка реестра. Замер: +0.6KB на
  ...
  //   каноне не тянет ни байта. Замер — +1.7KB на маршрут; берём 2KB, потому что половина
  //   бюджетов после прошлой волны снова стояла в пределах килобайта от потолка.
  // 2026-08-26: +2KB — общий вариант `row` у RailSection (анатомия разделов правой колонки, снятая
  //   замером с метрик Steep). Он живёт в components/metric/shared, а её импортируют ВСЕ страницы
  //   метрик, поэтому +0.45KB достаётся каждому маршруту, хотя пользуется им пока СДЭК — так и
  //   задумано: вариант общий и поедет на остальные вертикали. Этот маршрут стоял на 0.32KB запаса
  //   против 1.1–1.7KB у соседей и потому перевесил первым — это была не теснота кода, а
  //   мис-настроенный потолок (тот же случай, что у metrikaMetricCode тремя правками раньше).
  // 2026-08-31: +1KB маршрутам с источниками — ПЯТЫЙ источник (Rusender) в общем реестре
  //   сетей: две нав-строки за фичефлагом, бренд-глиф переключателя, иконка «конверт» и
  //   фильтр гейта в наве/палитре. Сами витрины Rusender — ленивый чанк, ни один байт их
  //   кода в эти маршруты не входит; растёт ровно оболочка реестра. Замер: +0.6KB на
