...
const BUDGETS = {
  ...
  //   каноне не тянет ни байта. Замер — +1.7KB на маршрут; берём 2KB, потому что половина
  //   бюджетов после прошлой волны снова стояла в пределах килобайта от потолка.
  // 2026-08-27: +1KB маршрутам С ГРАФИКАМИ — читалка (ChartTooltip) получила оценочную дельту
  //   сравнения со стрелкой и цветом и строку цели с отклонением от неё. Компонент дельты вынесен
  //   в свой лёгкий модуль (metric/comparisonDelta), чтобы читалка не тащила за собой весь модуль
  //   страницы метрики, но сам он теперь попадает в чанк КАЖДОЙ вертикали: замер +0.075KB. Столько
  //   запаса уже не было — после прошлых волн бюджеты стояли впритык (у «МойСклад метрика»
  //   оставался 21 БАЙТ), и первым потолок пробил «TG обзор». Берём 1KB, а не 0.1KB: половина
  //   маршрутов снова в сотнях байт от границы, и следующая правка общего графика упрётся туда же.
  overviewCode: 364 * KB,
