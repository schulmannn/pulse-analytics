...
Поиск упоминаний по умолчанию выключен из-за квоты Telegram `searchPosts`. Для локального collector
правила задаются явно в `.env` и не берутся из web-интерфейса:

```env
MENTION_QUERIES=brandname,brāndname,название бренда
MENTION_EXCLUDE=own_channel
```

После этого поиск включается только явной командой:
