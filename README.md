...
| `GDPR_EXPORT_MAX_CONCURRENT` | web | предел одновременных GDPR-экспортов (и не больше одного на пользователя), по умолчанию `2`, диапазон `1..8` и меньше `PGPOOL_MAX`; сверх лимита — `503` + `Retry-After` |
