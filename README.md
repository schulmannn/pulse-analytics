...
| `GDPR_EXPORT_DRAIN_TIMEOUT_MS` | web | сколько GDPR-экспорт ждёт, пока клиент заберёт очередной кусок ответа, прежде чем разорвать выгрузку и вернуть коннект в пул, мс, по умолчанию `60000`, диапазон `5000..600000` |
| `GDPR_EXPORT_MAX_CONCURRENT` | web | предел одновременных GDPR-экспортов (и не больше одного на пользователя), по умолчанию `2`, диапазон `1..8`; значение не меньше `PGPOOL_MAX` не валит старт, а зажимается до `PGPOOL_MAX − 1` (минимум 1) с предупреждением в логе; сверх лимита — `503` + `Retry-After` |
