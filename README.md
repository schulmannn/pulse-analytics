...
| `IG_BACKFILL_ENABLED` | web/worker | выключатель догрузки истории Instagram в архив `ig_daily` и дневного ремонта (`0`/`1`), по умолчанию `1`; дневной крон не задевает |
| `IG_BACKFILL_ACCOUNTS_PER_PASS` | web/worker | сколько IG-аккаунтов догрузка трогает за проход, по умолчанию `3`, диапазон `1..50` |
| `IG_BACKFILL_DAYS_PER_PASS` | web/worker | сколько дней истории одного аккаунта запрашивается за проход (≈11 вызовов Graph на день; дни, уже лежащие в архиве, бесплатны), по умолчанию `14`, диапазон `1..90` |
| `IG_BACKFILL_DAILY_CALLS` | web/worker | дневной бюджет вызовов Graph догрузки на аккаунт, по умолчанию `1500`, диапазон `11..100000` |
| `IG_BACKFILL_MAX_DAYS` | web/worker | как глубоко назад догрузка опрашивает Graph, дней, по умолчанию `730`, диапазон `1..3650`; это граница опроса, а не потолок чтения архива |
| `IG_BACKFILL_EMPTY_STREAK` | web/worker | сколько пустых дней подряд считаются горизонтом Graph (дальше не ходим), по умолчанию `7`, диапазон `1..90` |
| `IG_BACKFILL_TOPUP_DAYS` | web/worker | сколько последних дней (до вчера) раз в сутки перезапрашиваются финализированными значениями Meta, по умолчанию `2`, диапазон `0..7` |
| `IG_BACKFILL_BUC_STOP_PCT` | web/worker | мягкий стоп догрузки по заголовку Business-Use-Case-Usage, %, по умолчанию `75`, диапазон `1..100` |
| `IG_BACKFILL_PASS_BUDGET_MS` | web/worker | предел времени одного прохода догрузки, мс, по умолчанию `240000`, диапазон `10000..600000` |
