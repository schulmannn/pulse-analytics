...
- Баги/история/упоминания живут в Postgres; всё за `requireAuth` — только аккаунты
  (email+пароль/Google, RBAC по ролям из `users`); сессии подписывает `SESSION_SECRET`,
  web↔mtproto аутентифицируется отдельным `MTPROTO_TOKEN`.
