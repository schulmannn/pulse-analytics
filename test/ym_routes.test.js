...
test('summary (окно): точные ИТОГИ и качество из body.totals; дневные проценты НЕ суммируются', async () => {
  const { routes } = buildYm({
    ymFetch: async (_t, path) => {
      ...
      return {
        data: [
          { dimensions: [{ name: '2026-07-01' }], metrics: [30, 20, 60, 40, 90, 2, 12, 60, 3, 10] },
          { dimensions: [{ name: '2026-07-02' }], metrics: [70, 40, 190, 60, 100, 2.7, 22, 55, 7, 10] },
        ],
        // Дневные отказы были бы 40 и 60 — но период-отказ из totals = 45, НЕ 100 и НЕ сумма.
        // Порядок totals: visits,users,pageviews,bounce,avgDur,pageDepth,newUsers,pctNew,robotVisits,robotPct.
        totals: [100, 55, 250, 45, 96.4, 2.5, 34, 61.2, 10, 9.5],
      };
    },
  });
  ...
  assert.deepEqual(res.body.quality, {
    ...
    robot_visits: 10,       // явная роботность из totals[8]
    robot_percentage: 9.5,  // totals[9], round2
  });
  // Дневные серии качества выровнены по дате и берут дневные (не период) значения качества.
  assert.deepEqual(res.body.quality_series.bounce_rate, [
    { day: '2026-07-01', value: 40 },
    { day: '2026-07-02', value: 60 },
  ]);
  assert.deepEqual(res.body.quality_series.robot_percentage.map((p) => p.value), [10, 10]);
  assert.deepEqual(res.body.quality_series.robot_visits.map((p) => p.value), [3, 7]);
...
test('summary: при нулевых знаменателях доли/средние = null, счётчик новых = 0', async () => {
  ...
  // Роботность: доля при visits=0 → null; totals[8] отсутствует → robot_visits null (не 0).
  assert.equal(res.body.quality.robot_percentage, null);
  assert.equal(res.body.quality.robot_visits, null);
});
...
test('summary «Всё»: архивные серии + живое обогащение (точные итоги/качество), кэш обогащения 1ч', async () => {
  ...
  const { routes } = buildYm({
    ymFetch: async (_t, path) => {
      ...
      return { data: [], totals: [500, 300, 1200, 38.5, 84, 2.4, 210, 42, 25, 5] };
    },
    db: {
      getYmDailyAllForActor: async () => [
        {
          day: '2026-06-01', visits: 3, users: 2, pageviews: 5,
          bounce_rate: 40, avg_visit_duration_seconds: 70, page_depth: 1.67,
          new_users: 2, percent_new_visitors: 100, robot_visits: 1, robot_percentage: 33.33,
        },
        {
          day: '2026-06-02', visits: 4, users: 3, pageviews: 6,
          bounce_rate: 25, avg_visit_duration_seconds: 90, page_depth: 1.5,
          new_users: 1, percent_new_visitors: 33.33, robot_visits: 0, robot_percentage: 0,
        },
      ],
    },
  ...
  assert.equal(res.body.quality.robot_visits, 25);
  assert.deepEqual(res.body.quality_series.bounce_rate.map((p) => p.value), [40, 25]);
  assert.deepEqual(res.body.quality_series.robot_percentage.map((p) => p.value), [33.33, 0]);
  assert.equal(res.body.meta.exact_period_totals, true);
...
test('summary «Всё»: live-сбой не валит архив и negative-cache не долбит upstream повторно', async () => {
  ...
  assert.equal(first.body.quality.bounce_rate, null, 'периодный итог не выдумывается из дневной средней');
  assert.deepEqual(first.body.quality_series.robot_percentage, [{ day: '2026-06-01', value: 33.33 }]);
