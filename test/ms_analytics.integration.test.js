...
// Возвраты (ms_returns, миграция 032) — отдельный архив на ch (write-путь db.upsertMsReturns):
// count/sum/дневная серия точного окна, полная замена строки при корректировке, ForActor-гейт.
...
test.before(async () => {
  ...
  // Возвраты (ms_returns, миграция 032) — отдельный архив канала ch: два возврата agent-a, один
  // безагентный, один ВАЛИДНЫЙ 0-возврат (движок бы отбросил только неконечную сумму, не 0).
  const r = (return_id, moment, sum_kopecks, agent_id) => ({
    return_id, moment, sum_kopecks, agent_id, agent_name: agent_id ? `Имя ${agent_id}` : null,
  });
  await db.upsertMsReturns(ch.id, [
    r('ra1', '2026-01-12 10:00:00.000', 500, 'agent-a'),
    r('ra2', '2026-02-14 09:00:00.000', 300, 'agent-a'),
    r('rb1', '2026-02-14 11:00:00.000', 200, null),
    r('rc1', '2026-03-20 08:00:00.000', 0, 'agent-c'),
  ]);
  // Сосед-канал: его возврат НЕ должен просачиваться в агрегаты ch (tenant-изоляция по каналу).
  await db.upsertMsReturns(other.id, [r('rz1', '2026-03-05 10:00:00.000', 999900, 'agent-z')]);
});
...
// ── Возвраты (ms_returns, миграция 032) ───────────────────────────────────────────────────────

test('getMsReturnsInternal (вся история): точный count/sum и дневная серия; валидный 0-возврат в счёте, сосед изолирован', { skip }, async () => {
  const data = await db.getMsReturnsInternal(ch.id, {});
  assert.equal(data.count, 4, 'ra1+ra2+rb1+rc1 (0-возврат считается)');
  assert.equal(data.sum_kopecks, 1000, '500+300+200+0');
  assert.deepEqual(data.series, [
    { day: '2026-01-12', count: 1, sum_kopecks: 500 },
    { day: '2026-02-14', count: 2, sum_kopecks: 500 },   // ra2+rb1
    { day: '2026-03-20', count: 1, sum_kopecks: 0 },
  ]);
  // Кривой sinceDay не долетает до SQL (repo не доверяет вызывающему) — трактуется как вся история.
  assert.deepEqual(await db.getMsReturnsInternal(ch.id, { sinceDay: 'DROP TABLE' }), data);
});

test('getMsReturnsInternal (окно): sinceDay/untilDay режут по календарному дню moment (инклюзивно)', { skip }, async () => {
  const data = await db.getMsReturnsInternal(ch.id, { sinceDay: '2026-02-01', untilDay: '2026-02-28' });
  assert.equal(data.count, 2);           // ra2, rb1
  assert.equal(data.sum_kopecks, 500);
  assert.deepEqual(data.series, [{ day: '2026-02-14', count: 2, sum_kopecks: 500 }]);
});

test('upsertMsReturns: полная замена строки (правка вниз и снятие контрагента), не COALESCE', { skip }, async () => {
  try {
    // Корректировка: сумма ra1 упала 500→100, контрагент снят (agent_id/name → null).
    await db.upsertMsReturns(ch.id, [
      { return_id: 'ra1', moment: '2026-01-12 10:00:00.000', sum_kopecks: 100, agent_id: null, agent_name: null },
    ]);
    const data = await db.getMsReturnsInternal(ch.id, {});
    assert.equal(data.count, 4, 'по-прежнему 4 документа (upsert по PK, не вставка дубля)');
    assert.equal(data.sum_kopecks, 600, 'сумма честно уменьшилась (500→100): 100+300+200+0');
    const { rows } = await pool.query('SELECT agent_id FROM ms_returns WHERE channel_id=$1 AND return_id=$2', [ch.id, 'ra1']);
    assert.equal(rows[0].agent_id, null, 'снятый контрагент занулён, а не оставлен старым');
  } finally {
    // Восстанавливаем сид для независимости остальных тестов набора.
    await db.upsertMsReturns(ch.id, [
      { return_id: 'ra1', moment: '2026-01-12 10:00:00.000', sum_kopecks: 500, agent_id: 'agent-a', agent_name: 'Имя agent-a' },
    ]);
  }
});

test('getMsReturnsForActor: владелец видит агрегат, чужой actor → null (репо гейтит сам)', { skip }, async () => {
  const ownerData = await db.getMsReturnsForActor(ch.id, { uid: owner.id }, {});
  assert.equal(ownerData.count, 4, 'владелец видит возвраты');
  assert.equal(await db.getMsReturnsForActor(ch.id, { uid: stranger.id }, {}), null, 'чужой → null');
});
