...
test('customers: дневные суммы конвертируются в рубли, окно и actor доходят до tenant-scoped repo', async () => {
  const { routes } = buildMs({
    msFetch: async () => { throw new Error('customers не ходит в живой МойСклад'); },
    db: {
      getMsCustomersForActor: async (channelId, actor, opts) => {
        assert.equal(channelId, 5);
        assert.equal(actor.uid, 7);
        assert.match(opts.sinceDay, /^\d{4}-\d{2}-\d{2}$/);
        assert.match(opts.untilDay, /^\d{4}-\d{2}-\d{2}$/);
        return {
          summary: {
            customers: 2, new_customers: 1, repeat_customers: 1,
            orders_new: 1, orders_repeat: 2,
            sum_new_kopecks: 12345, sum_repeat_kopecks: 67890,
            no_agent_orders: 0, repeat_ever: 1,
          },
          series: [
            {
              day: '2026-07-17', new_orders: 1, repeat_orders: 2,
              sum_new_kopecks: 12345, sum_repeat_kopecks: 67890,
            },
          ],
        };
  ...
  const res = await invoke(routes, 'GET /api/ms/customers', { query: { days: '30' } });
  assert.equal(res.statusCode, 200);
  assert.deepEqual(res.body.series, [
    { day: '2026-07-17', new_orders: 1, repeat_orders: 2, sum_new: 123.45, sum_repeat: 678.9 },
  ]);
...
test('top-products: полный raw-отчёт сортируется до limit и переиспользуется между метриками', async () => {
  ...
  const { routes, sleeps } = buildMs({
    msFetch: async (_token, path) => {
      ...
      return Number(q.get('offset')) === 0
        ...
        : {
            rows: [
              { assortment: { name: 'Я-хит' }, sellQuantity: 5, sellSum: 999_00, profit: 100_00 },
              { assortment: { name: 'Я-прибыль' }, sellQuantity: 2, sellSum: 300_00, profit: 200_00 },
              { assortment: { name: 'Я-маржа' }, sellQuantity: 1, sellSum: 10_00, profit: 9_00 },
              { assortment: { name: 'Я-без-выручки' }, sellQuantity: 1, sellSum: 0, profit: 800_00 },
            ],
          };
    },
  });
  ...
  assert.equal(res.body.rows[0].margin, 10.01, 'маржа = profit/revenue, округление только на выходе');

  const profit = await invoke(routes, 'GET /api/ms/top-products', {
    query: { days: '30', limit: '2', sort: 'profit' },
  });
  assert.deepEqual(profit.body.rows.map((r) => r.name), ['Я-без-выручки', 'Я-прибыль']);
  assert.equal(profit.body.rows[0].margin, null, 'нулевая выручка не превращается в выдуманную маржу');

  const margin = await invoke(routes, 'GET /api/ms/top-products', {
    query: { days: '30', limit: '2', sort: 'margin' },
  });
  assert.deepEqual(margin.body.rows.map((r) => r.name), ['Я-маржа', 'Я-прибыль']);

  const invalid = await invoke(routes, 'GET /api/ms/top-products', {
    query: { days: '30', limit: '1', sort: 'unexpected' },
  });
  assert.equal(invalid.body.rows[0].name, 'Я-хит', 'неизвестный sort безопасно возвращается к выручке');
...
test('top-products: days=0 — окно от первого дня месяца старейшего заказа архива; кэш-ключ различает 0 и 30', async () => {
  ...
  assert.equal(res.statusCode, 200);
  assert.deepEqual(res.body.rows, [{ name: 'Товар', quantity: 1, revenue: 100, profit: 10, margin: 10 }]);
