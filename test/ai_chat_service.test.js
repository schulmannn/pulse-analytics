...
test('get_sklad_funnel: definition обещает только распределение по последнему сохранённому статусу', () => {
  const tools = createAiTools({ db: skladDb() });
  const definition = tools.definitions.find((item) => item.name === 'get_sklad_funnel');
  assert.ok(definition);
  assert.match(definition.description, /созданных за период.*последнему сохранённому статусу/i);
  assert.match(definition.description, /НЕ исторический статус на конец периода/i);
  assert.match(definition.description, /не переходы между статусами, не конверсия/i);
  assert.doesNotMatch(definition.description, /Воронка статусов/i);
});
...
test('get_sklad_funnel: имена статусов из живого словаря; без аккаунта — id + пометка', async () => {
  const rows = [
