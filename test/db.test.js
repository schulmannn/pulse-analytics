...
// Ковчег-инвариант: views_graph — ИНКРЕМЕНТАЛЬНЫЙ (не кумулятив). Трансформ маппит x[i]→values[i]
// по дню и НЕ суммирует; повторный прогон тех же graphs даёт идентичные строки (никакого
// накопления), а ON CONFLICT в upsert перезаписывает день → no double-count при ретрае крона.
test('graphsToDailyRows: инкрементальный, не кумулятив; идемпотентен при повторе', () => {
  const graphs = {
    available: true,
    interactions: { x: [T1, T2], series: [{ name: 'views', values: [1000, 1200] }] },
  };
  const a = db.graphsToDailyRows(graphs);
  const b = db.graphsToDailyRows(graphs);          // повторный прогон
  assert.deepStrictEqual(a, b, 'тот же вход → те же строки (нет накопления состояния)');
  // День 2 = его собственное значение (1200), НЕ сумма с днём 1 (2200) — данные уже дневные.
  assert.strictEqual(rowOf(a, '2026-06-02').views, 1200);
  assert.strictEqual(rowOf(a, '2026-06-01').views, 1000);
});
