...
test('гибрид TG+IG: канал с привязанным IG принимает и tg-, и ig-посты', { skip }, async () => {
  // Регресс: у TG-канала с привязанным Instagram-аккаунтом source_id остаётся TG-источником
  // (IG-сторона живёт на ig_accounts.source_id, см. миграцию 010). Раньше репо выводил ОДНУ
  // каноническую сеть = 'tg' и отбивал добавление ig-поста как «неверную платформу».
  const hybrid = await db.createTgChannel({
    owner_uid: S.userA.id, tg_channel_id: Date.now() + 11, username: `hyb_${nonce}`, title: 'Hybrid A',
  });
  await pool.query(
    `INSERT INTO ig_accounts (channel_id, ig_user_id, username, access_token_enc, updated_at)
     VALUES ($1,$2,$3,'x:x:x', now()) ON CONFLICT (channel_id) DO NOTHING`,
    [hybrid.id, `ig_${nonce}`, `hyb_${nonce}`]);
  await pool.query(
    `INSERT INTO posts (post_id, channel_id, date_published, views, media_type, caption)
     VALUES (501, $1, '2026-06-11T08:00:00Z', 500, 'photo', 'hybrid tg post')
     ON CONFLICT (channel_id, post_id) DO NOTHING`, [hybrid.id]);
  const camp = await db.createCampaign(S.userA.id, { channel_id: hybrid.id, name: `Гибрид ${nonce}` });
  try {
    const res = await db.addCampaignPosts(S.userA.id, camp.id, [
      { network: 'tg', channel_id: hybrid.id, post_ref: '501' },
      { network: 'ig', channel_id: hybrid.id, post_ref: `hyb_media_${nonce}`, published_at: '2026-06-11T10:00:00Z' },
    ]);
    assert.strictEqual(res.added, 2, 'один channel_id принимает и tg-, и ig-посты');
    const rows = await db.listCampaignPosts(S.userA.id, camp.id);
    assert.deepStrictEqual(new Set(rows.map((row) => row.network)), new Set(['tg', 'ig']));
  } finally {
    await db.deleteCampaign(S.userA.id, camp.id);
  }
});
...
test('summary: точный срез по источнику меняет КАЖДЫЙ агрегат', { skip }, async () => {
  const full = await db.getCampaignSummary(S.userA.id, S.campaign.id);

  // Срез на (tg, tgA): только три поста tgA — сумма 9000 против общей 13000 (без hidden-202).
  const tgScope = await db.getCampaignSummary(S.userA.id, S.campaign.id, { network: 'tg', channelId: S.tgA.id });
  assert.strictEqual(tgScope.posts_total, 3);
  assert.strictEqual(tgScope.inaccessible_posts, 0);
  assert.strictEqual(tgScope.tg.posts, 3);
  assert.strictEqual(tgScope.tg.views, 9000);
  assert.notStrictEqual(tgScope.tg.views, full.tg.views, 'срез меняет сумму просмотров');
  assert.strictEqual(tgScope.tg.median, 2000);
  assert.strictEqual(tgScope.tg.avg, 3000);
  assert.strictEqual(tgScope.ig.posts, 0, 'ig-платформа вне среза пуста');
  assert.strictEqual(tgScope.by_source.length, 1);
  assert.strictEqual(tgScope.by_source[0].channel_id, S.tgA.id);
  assert.strictEqual(tgScope.by_source[0].tg_views, 9000);
  assert.deepStrictEqual(
    tgScope.by_format.map((row) => [row.media_type, row.posts]),
    [['photo', 2], ['video', 1]],
    'форматы считаются только внутри выбранной пары');
  assert.strictEqual(tgScope.timeline.length, 3);
  assert.deepStrictEqual(tgScope.timeline.map((row) => row.tg_views), [1000, 2000, 6000]);
  assert.strictEqual(tgScope.tg.best.ratio, 3, 'best/worst пересчитываются относительно медианы среза');
  assert.strictEqual(tgScope.tg.worst.ratio, 0.5);
  assert.strictEqual(tgScope.comparison.available, true, 'baseline строится для того же TG-источника');

  // Срез на (ig, igA): только ig-пост, tg-платформа пуста.
  const igScope = await db.getCampaignSummary(S.userA.id, S.campaign.id, { network: 'ig', channelId: S.igA.id });
  assert.strictEqual(igScope.posts_total, 1);
  assert.strictEqual(igScope.ig.posts, 1);
  assert.strictEqual(igScope.ig.reach, 800);
  assert.strictEqual(igScope.ig.views, 1200);
  assert.strictEqual(igScope.tg.posts, 0);
  assert.strictEqual(igScope.by_source.length, 1);
  assert.strictEqual(igScope.by_source[0].channel_id, S.igA.id);
  assert.strictEqual(igScope.by_format.length, 1);
  assert.strictEqual(igScope.timeline.length, 1);
  assert.strictEqual(igScope.comparison.available, false);

  // Мисматч network/channel_id (в кампании нет ig-постов канала tgA) → честно пусто.
  const mismatch = await db.getCampaignSummary(S.userA.id, S.campaign.id, { network: 'ig', channelId: S.tgA.id });
  assert.strictEqual(mismatch.posts_total, 0);
  assert.strictEqual(mismatch.ig.posts, 0);
  assert.deepStrictEqual(mismatch.by_format, []);
  assert.deepStrictEqual(mismatch.timeline, []);
  assert.strictEqual(mismatch.comparison.available, false);
});

test('summary: срез на недоступный/чужой источник не раскрывает данные', { skip }, async () => {
  // tgB принадлежит userB и не входит в кампанию: угаданный ID обязан вернуть честную пустую
  // сводку той же формы — без метрик, без факта существования постов у этого источника.
  const scoped = await db.getCampaignSummary(S.userA.id, S.campaign.id, { network: 'tg', channelId: S.tgB.id });
  assert.ok(scoped && scoped.campaign, 'сводка отдаётся (доступ к кампании есть), но пустая');
  assert.strictEqual(scoped.posts_total, 0);
  assert.strictEqual(scoped.inaccessible_posts, 0, 'недоступный источник не всплывает даже как счётчик');
  assert.strictEqual(scoped.tg.posts, 0);
  assert.strictEqual(scoped.tg.views, null);
  assert.strictEqual(scoped.ig.posts, 0);
  assert.strictEqual(scoped.by_source.length, 0);
  assert.strictEqual(scoped.timeline.length, 0);
  assert.strictEqual(scoped.comparison.available, false);
});
