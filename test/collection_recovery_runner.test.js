...
function makeRunner(over = {}) {
  ...
  const runner = createCollectionRecoveryRunner({
    ...
    // Тот же null/undefined-протокол для МС-прохода (optional dep с инертным дефолтом).
    ...(over.runMsCollectionPass === null
      ? {}
      : { runMsCollectionPass: over.runMsCollectionPass
          || (async () => { msCalls.push(true); return { channels: 0 }; }) }),
    // …и для прохода заказов МС (resume бэкфиллов + дневная доливка) — та же ms-lane.
    ...(over.runMsOrdersPass === null
      ? {}
...
test('МС-проход едет тем же планировщиком: зовётся в каждом проходе бегунка', async () => {
  const { runner, clock, msCalls, msOrdersCalls, igCalls } = makeRunner();
  runner.start();
  await clock.fireNext();
  assert.deepEqual(msCalls, [true], 'ms-lane выполнена в проходе');
  assert.deepEqual(msOrdersCalls, [true], 'проход заказов (resume+topup) едет той же lane');
  assert.deepEqual(igCalls, [25], 'IG-проход не задет добавленной lane');
  assert.equal(clock.pending, 1, 'после прохода запланирован следующий (тот же интервал)');
  assert.equal(clock.lastDelay, 5000);
});

test('сбой МС-прохода изолирован: IG и TG всё равно отрабатывают', async () => {
  const { runner, clock, igCalls, tgCalls } = makeRunner({
    runMsCollectionPass: async () => { throw new Error('ms boom'); },
  });
  runner.start();
  await assert.doesNotReject(clock.fireNext());
  assert.deepEqual(igCalls, [25], 'IG-проход не задет падением ms-прохода');
  assert.deepEqual(tgCalls, [200], 'TG-проход не задет падением ms-прохода');
  assert.equal(clock.pending, 1, 'следующий проход всё равно запланирован');
});

test('МС-проход не инъектирован → дефолтный no-op, проход не падает', async () => {
  const { runner, clock, igCalls } = makeRunner({ runMsCollectionPass: null });
  runner.start();
  await assert.doesNotReject(clock.fireNext());
  assert.deepEqual(igCalls, [25]);
});

test('ms-lane: заказы (resume+topup) идут в каждом проходе ПОСЛЕ дневного архива, последовательно', async () => {
  let releaseMs;
  const gate = new Promise((resolve) => { releaseMs = resolve; });
  const order = [];
  const { runner } = makeRunner({
    runMsCollectionPass: async () => { order.push('ms:start'); await gate; order.push('ms:end'); return {}; },
    runMsOrdersPass: async () => { order.push('orders'); return {}; },
  });
  const pass = runner.runOnce();
  await Promise.resolve();
  await Promise.resolve();
  assert.deepEqual(order, ['ms:start'], 'проход заказов не конкурирует с активным дневным архивом (один лимит МС)');
  releaseMs();
  await pass;
  assert.deepEqual(order, ['ms:start', 'ms:end', 'orders']);
});

test('сбой прохода заказов МС изолирован: IG/TG/дневной архив всё равно отрабатывают', async () => {
  const { runner, clock, igCalls, tgCalls, msCalls } = makeRunner({
    runMsOrdersPass: async () => { throw new Error('orders boom'); },
  });
  runner.start();
  await assert.doesNotReject(clock.fireNext());
  assert.deepEqual(igCalls, [25], 'IG-проход не задет падением orders-прохода');
  assert.deepEqual(tgCalls, [200], 'TG-проход не задет падением orders-прохода');
  assert.deepEqual(msCalls, [true], 'дневной архив МС не задет падением orders-прохода');
  assert.equal(clock.pending, 1, 'следующий проход всё равно запланирован');
});

test('сбой дневного архива МС не отменяет проход заказов той же lane', async () => {
  const { runner, clock, msOrdersCalls } = makeRunner({
    runMsCollectionPass: async () => { throw new Error('ms boom'); },
  });
  runner.start();
  await assert.doesNotReject(clock.fireNext());
  assert.deepEqual(msOrdersCalls, [true], 'resume+topup выполняются даже после сбоя архива');
});

test('проход заказов МС не инъектирован → дефолтный no-op, проход не падает', async () => {
  const { runner, clock, igCalls, msCalls } = makeRunner({ runMsOrdersPass: null });
  runner.start();
  await assert.doesNotReject(clock.fireNext());
  assert.deepEqual(igCalls, [25]);
