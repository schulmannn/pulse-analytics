...
| **Cron double-fire** | run heavy pass once/day | **was:** two `/graphs`+`/posts`+velocity passes + racing upserts. **now:** `runJobOnce('daily_ingest','central:<UTC-date>')` → one caller works, duplicate returns cached result + skips the tails | 200 `{skipped:true}` on dup | unchanged | **FIXED (PR #39)** |
| **Ingest degraded (MTProto down @ cron)** | archive stops growing LOUDLY, not silently | **was:** `/graphs`→null → `channel_daily=0` → `{ok:true}`/HTTP 200 → cron green, archive rots unseen (only DataHealth's `stale≥2d` warned, and only if a human opened the dashboard). **now:** endpoint sets `degraded:true` + `log('error','ingest_degraded')`; the cron greps the flag and `exit 1` → GitHub's native workflow-failure email fires | 200 `{degraded:true}` | DataHealth stale badge (unchanged) | **FIXED (this PR)** — proactive channel = GitHub failure email; residual: GitHub disables schedules after 60d repo-inactivity (see ADR-002 §Observability) |
