...
- [x] **Обзор: Инсайт vs леджер спорили о подписчиках — SHIPPED.** Digest суммировал
      joined−left по ВСЕМУ окну TG-графа (не по периоду), леджер — `subscriberChange` по
      архиву за период. Теперь Digest берёт ровно `subscriberChange(history.rows, days)`
      (тот же источник и оконная математика, что у леджера); кастомный диапазон — клауза
      опускается (как и Δ в леджере); скелетон ждёт и history.
...
- [x] **IG-леджер «0 · ↓100%» у мёртвых метрик — SHIPPED.** `isLive(pair)= hasCur && cur>0`
      в IgOverview: Просмотры/Взаимодействия при окне без единого ненулевого замера → «—»
      без дельты (нет данных ≠ ноль); erTrend дополнительно гейтится `erReach>0` (раньше
      «—» могло соседствовать с «↓100%»). Hint про API-квоту не добавлял — прочерк молчит.
...
- [x] **Markdown-лик в «Топ постов» — SHIPPED (оба слоя).** (а) сервер: `lib/caption.js
      captionSnippet()` — стрип inline-подмножества (link/bold/underline/code/italic +
      сиротские `**`) ДО усечения; оба пути записи (`index.js` крон-ингест и
      `collector/contract.js`) на нём. Старые строки в БД самоизлечатся ближайшим
      апсертом крона. (б) клиент: `repairTruncatedTail` в `parseInlineMarkdown` — висячий
      `[label](url-cut` / `[label-cut` в САМОМ конце → label (только link-shaped: хвостовые
      `*`/`` ` `` не трогаем — это легитимные закрывающие маркеры). + тесты.
- [x] **Склейка строк «мастерской Именно» — SHIPPED.** `markdownToPlainText`: `\n+` → « · »
      (вместо схлопывания в пробел); все титулы (TopPosts/Posts/Digest) идут через неё —
      отдельный `postTitle()` не понадобился.
- [x] **Модалка поста: мета-сокращения — закрыто мерджем модалок.** PostModal удалена; в
      общей модалке стат-грид с полными словами («Просмотры/Реакции/Репосты/Комментарии»).
- [x] **ДВЕ модалки поста слиты — SHIPPED.** `PostDetailModal` = единственная: фото + титул
      (RichText) + стат-гриды + «Динамика набора просмотров» (`PostVelocity`, поглощена из
      PostModal; тихо прячется без стат-доступа) + реакции + хэштеги + «Открыть в Telegram».
      `rank` стал опциональным (таблица «Публикаций» его не передаёт — № зависит от текущей
      сортировки); `panels/Posts.tsx` PostModal удалена (~130 строк).
...
- [x] **`fmt.day` сдвигал даты на день в западных таймзонах — SHIPPED.** Причина: date-only
      строки парсились как UTC-полночь и рендерились в локальной TZ → в UTC−3 «2026-06-01»
      читалось «31 мая» во всех осях/таблицах. Фикс: `parseDayKey` в lib/format.ts — bare
      `YYYY-MM-DD` = ЛОКАЛЬНАЯ полночь (календарная дата, не instant; во всех TZ читается
      одинаково), full-ISO/Date/epoch остаются instant-семантикой (локальный день момента);
      `fmt.day` теперь принимает и Date/epoch — двойные сдвиги в чипах диапазона
      (MetricPage/ReportPage `new Date(ms).toISOString().slice(0,10)`) заменены прямым
      `fmt.day(range.from)`; «сгенерирован» в отчёте = `fmt.day(new Date())` (ручная сборка
      локального ключа больше не нужна); `fmt.date` для bare-ключа не выдумывает время
      (раньше «...,00:00» уезжало в «21:00 вчера»). +7 юнит-тестов (TZ-независимые по
      построению). **Сверка с бакетированием:** конвенция подтверждена и оставлена —
      ключи = UTC-дни (совпадают с серверными/TG), вся математика по ключам UTC
      (kpiDerive/MetricPage bucketKeyOf/ReportPage mondayKey/delta), пост 22:00 UTC−3
      остаётся в UTC-завтра (консистентно с архивом); Charts-heatmap «лучшее время»
      осознанно локальная (вопрос «когда постить» — в часах зрителя).
...
- [x] **FLIP-анимация перетаскивания виджетов (SHIPPED).** Реордер (drag и «Выше/Ниже»)
      больше не телепортирует: WidgetGroup снапшотит позиции всех зарегистрированных нод в
      момент store-notify (синхронно ДО ре-рендера), в useLayoutEffect после коммита ставит
      обратный translate → reflow → transition 260ms cubic-bezier(.2,.7,.3,1) → виджеты
      СКОЛЬЗЯТ на новые места; mid-flight снапшоты дают плавное продолжение (rect включает
      текущий transform); prefers-reduced-motion выключает. Для этого ChartSection разделён
      на два слоя: ВНЕШНЯЯ section = grid-order + FLIP-translate (+ drag-хендлеры), ВНУТРЕННИЙ
      div = карточка (bg/border/паддинги/акцент/тинт/джигл) — rotate-анимация джигла на том же
      элементе стоптала бы FLIP-translate. Регистрация в группе теперь несёт DOM-ноду.

- [x] **Drag v2: плашка следует за курсором (pointer events вместо HTML5 DnD, SHIPPED).**
      Живой глаз-тест HTML5-версии: сама плашка стояла в слоте (opacity-50), у курсора ехала
      только браузерная полупрозрачная «тень» (drag image), реордер читался лишь в конце —
      выглядело как «FLIP мёртв». Переписано на pointerdown/move/up + setPointerCapture:
      карточка «приклеивается» к курсору (translate на внешнем слое; после каждого коммита
      реордера и на автоскролле — rebase к последней позиции указателя), порог 5px, подъём =
      scale(1.02) на внутреннем слое + пауза собственного джигла + shadow-lg (транзиентный
      лифт, не статичный хром) + z-30; соседи разъезжаются прежним FLIP. Хит-тест
      геометрический по зарегистрированным нодам; виджеты в полёте (`data-gliding`)
      нетаргетируемы — анти-пинг-понг вместо сломанного pair-guard (тот сравнивал `A>B` с
      `B>A` и не срабатывал никогда). Отпускание/Esc/«Готово» — доглаживание в слот тем же
      изингом (+таймаут-страховка 400мс на проглоченный transitionend). Бонусы: **drag
      заработал на таче** (`touch-none`, Выше/Ниже остаются как precision-фоллбэк),
      edge-автоскролл у краёв вьюпорта (interval 16мс, НЕ rAF — headless-гочи), противофаза
      джигла вернулась инлайн-фазой по чётности слота (`:nth-child(even)` умер при разделении
