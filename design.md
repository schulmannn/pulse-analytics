...
### D5.1 Полировка-баги (P0)
- [ ] Markdown-сырец в текстах постов (`[текст](url)`, `__…__`, `**…**` видны как есть) — стрип TG-entities в превью (Топ постов / Посты / модалка; `lib/posts` normalize).
- [ ] «обновлено обновлено сегодня»: `freshness()` уже включает префикс (`lib/freshness.ts:39`), потребители дублируют — `components/DataHealth.tsx:74`, `DashboardLayout.tsx:271,282`, `panels/Overview.tsx:78`.
- [ ] Двойной H1 «Настройки» (топбар + тело страницы).
- [ ] Английский в графиках: серии `Views`/`Shares` из API рендерятся как есть (`panels/TgAnalytics.tsx:347-348`), оси «18 May / 24 Jun 21:00» с graphs-API; заголовок модалки «Shares». Локализовать series-map + форматировать оси на клиенте.
- [ ] Тултип LineChart: перекрывает шапку страницы + залипает после ухода курсора.
- [ ] Y-подписи внутри плота (перекрыты линией/заливкой: «4.8k», «47», «189») — отступ под ось.
- [ ] ExpandableChart: клик по всему графику открывает модалку — конфликт с crosshair; разворот только по ↗.
- [ ] «Аудитория»: дыра в 2-col сетке (левая колонка короче правой на два чарта).
- [ ] BarChart n=2 → столбы во всю ширину (IG Reels) — max-width бара ~48px.
- [ ] IG: пустое видео-превью (белая карточка) + рваная высота обложек в гриде.
- [ ] Данные спорят: Обзор-инсайт «+5.0% к прошлому периоду» vs Сравнение «Недостаточно истории» — свести источники/копи.
- [ ] Error-карточка без «Повторить»; при отказе API аноним видит её вместо лендинга.

### D5.2 Chart-craft 2 + язык компонентов (P1)
- [ ] Убрать вертикальный градиент столбиков → плоская заливка + hover (главный «шаблонный» приём сейчас).
- [ ] Топ постов: «▲ +N% к среднему…» под каждой строкой → только реальным выбросам (`panels/TopPosts.tsx:74`).
- [ ] ERV/ER-радуга в Посты: красить только вне нормального диапазона (PctTag-пороги).
- [ ] Язык кнопок: «Обновить» (Mentions), «Сменить фото/Удалить», «Подключить Instagram» — прямоугольники при системе CTA=pill. Свести.
- [ ] Dark: болотные tint-подсветки строк (chart-2 orange на тёмном) в Breakdown/По форматам — отдельные dark-значения.
- [ ] Rail: тултипы на иконках свёрнутого сайдбара; заметный пин «Развернуть».
- [ ] «UID: 1 · Owner: 1» в Настройках — спрятать за админ-режим.
- [ ] Пустое «Период vs предыдущий» → EmptyState-паттерн (`panels/Compare.tsx`).
- [ ] Модалка разворота бар-чартов: добавить оси/значения или не разворачивать бары.
- [ ] Auth-страницы: заполнить пустую половину (мини-скрин продукта/цитата) + проверить `GOOGLE_CLIENT_ID` на проде (кнопки Google нет).

### D5.3 Паттерны из steep (адаптировать под Refined Technical)
- [ ] **Period pager** «‹ 30д ›» рядом с пресетами периодов — листать окна назад/вперёд.
- [ ] **Value labels** на пике/последней точке графика (не только hover).
- [ ] **Состояние в URL** (период/таб/сорт) — шарабельные ссылки.
- [ ] **Метрика как объект** (эволюция KpiDrillDown → страница метрики: определение простым языком + breakdown + compare). Большой, обсудить скоуп.
