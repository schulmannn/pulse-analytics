...
- [ ] Y-ось LineChart: nice-ticks. Сейчас: дубль «4.9k/4.9k/4.8k» (Рост), неровные шаги
      «5.3k/5.1k/4.8k» (История), «262» рядом с «2.5k» (IG hero), min-тик «1» вместо 0
      (модалка поста). Алгоритм: шаг 1/2/5×10ⁿ, старт от 0 для объёмных метрик (steep:
      `y-zoom=start-zero`), формат единый для всех тиков, дедуп после форматирования.
...
- [ ] Командная палитра: строка-заголовок «Командная палитра» + × (`CommandPalette.tsx:133`)
      → безголовый input-first (Linear/Raycast); добавить IG-команды и hint «⏎ открыть · Esc».
...
- [ ] **Metric page (S2) — модель уточнена по live-обходу**: полноэкранный чарт метрики +
      правая Explore-панель (Breakdown / Filter / Compare / Targets аккордеоном) + таб About
      (определение простым языком + владелец) + нижний таймбар (пресеты + «‹ период ›» pager).
      Наш путь: KpiDrillDown → `/metric/:key` с этой раскладкой.
- [ ] **Report-документ (S4/F3)**: глобальный фильтр-бар пилюлями (Период · Канал · Платформа)
      над документом, метрик-карты + текст-секции; «Add to report» с любой метрики.
...
- [ ] Политика «ось от нуля» для объёмных метрик (просмотры/подписчики per-post) — как
      дефолт LineChart/BarChart с явным opt-out.
