...
function createAnalyticsRepo({ pool, enabled, getAccessibleChannel }) {
  ...
  // МойСклад — свой репозиторий (аудит #554): своя таблица, свои CTE, своя денежная
  // единица. Гейт доступа приходит отсюда тем же объектом — второй копии правила не заводим.
  const ms = createMsAnalyticsRepo({ pool, enabled, gated, LIST, NONE });
  ...
  return {
    // Полоса МойСклада приходит спредом из msAnalyticsRepo — форма фасада не меняется.
    ...ms,
