...
/* ── Rusender: учётка источника, архив рассылок и дневная активность (миграция 039) ─────────────
   ...
   ЗАМЕР НА ЖИВЫХ ДАННЫХ (все три запущенные рассылки аккаунта владельца) подтвердил, что
   расхождение реально и системно:

     рассылка   ряд              итоги            точек
     108243     128 оп / 12 кл   134 оп / 13 кл   11
     101284     124 оп / 26 кл   128 оп / 30 кл   11
     76934        1 оп /  0 кл     1 оп /  0 кл   11

   Точек РОВНО 11 у каждой, независимо от даты отправки (январь и июль) — значит окно активности
   Rusender фиксированное, 11 дней от отправки, а не «сколько накопилось». Хвост открытий за
   пределами окна попадает только в кумулятивные итоги; у крошечной рассылки без хвоста ряд и
   итоги сходятся точно. «Дотягивать» ряд до итогов НЕЛЬЗЯ: ряд честно отвечает на вопрос
   «когда читали», итоги — «сколько всего прочитали».
...
// «Свежая» рассылка: запущена не позже стольких дней назад ИЛИ ещё идёт. Замер показал, что окно
// активности у Rusender фиксированное — 11 дней от отправки (см. шапку файла), то есть после
// ~11 дней новых точек у рассылки не появится вовсе. 30 оставлено СОЗНАТЕЛЬНО как безопасное
// надмножество: окно замерено на одном аккаунте, и запас в две недели дешевле, чем риск
// перестать добирать хвост, если у Rusender оно окажется шире. Архив всё равно вращается.
