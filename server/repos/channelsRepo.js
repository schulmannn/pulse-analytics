...
function createChannelsRepo({ pool, enabled, transaction, ensureExternalSource }) {
  ...
  // Standalone-канал СДЭК Fulfillment (source='cdek', username NULL), зеркально createMsChannel.
  // Пары findCdekChannelBy* здесь НЕТ, и это осознанно: у источника с ручной загрузкой Excel нет
  // внешней идентичности, по которой его можно было бы узнать при повторном «подключении» —
  // источник заводится руками один раз и живёт под своим именем (см. комментарий в cdekRepo).
  async function createCdekChannel({ owner_uid, name }) {
    if (!enabled || owner_uid == null) return null;
    const title = String(name || '').trim() || 'СДЭК';
    // Finding 2: канал + workspace-привязка одной транзакцией (см. createChannel).
    return transaction(async (client) => {
      const { rows } = await client.query(
        `INSERT INTO channels (owner_uid, username, title, status, source)
         VALUES ($1,NULL,$2,'active','cdek') RETURNING ${CHANNEL_COLS}`,
        [owner_uid, title]);
      const row = rows[0] || null;
      // Workspace сейчас; канонический cdek-source штампует saveCdekSource (id канала известен там).
      if (row) await ensureChannelCanonical(row.id, owner_uid, {}, client);
      return row;
    });
  }
