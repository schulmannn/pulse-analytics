...
function registerCdekRoutes({ app, express, requireAuth, db, audit, cdekImport }) {
  ...
  //
  // Этот потолок ограничивает ОДНУ загрузку; сколько исходников канал держит в базе, ограничивает
  // окно хранения `CDEK_IMPORT_FILES_KEPT` в repos/cdekRepo (M-1). Числа перемножаются: 2 МБ × 10
  // = 20 МБ на канал в худшем случае. Меняя одно, посмотри на второе — порознь они не значат ничего.
