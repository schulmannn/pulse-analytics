...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:100%;background:#ffffff;border:1px solid #e4e2db;border-radius:12px">
  ...
  /* ── Приём приглашения БЕЗ аккаунта ───────────────────────────────────────────────────────────
     ...
     открыть ссылку снова. У 'unverified' пароль перезаписывается ВСЕГДА, потому что засеять такую
     строку заранее мог атакующий: на нераскрытой ссылке — выбранным сейчас, на раскрытой —
     неиспользуемым (тот же разбор, что в google-ветке auth.js). */
  app.post('/api/team/invite/:token/claim', authLimiter, async (req, res, next) => {
    ...
    try {
      ...
      let user;
      if (existing) {
        // Пароль ЗАСЕЯННОЙ строки переписывается ВСЕГДА — расходится только чем.
        //
        // Первая правка (#558) на раскрытой ссылке не трогала его вовсе: «перезаписать его мог бы
        // ровно тот, кто ссылку и раскрыл». Рассуждение верное, а вывод из него — нет. Пароль в
        // такой строке УЖЕ чужой: атакующий регистрирует чужой email через /register (строка
        // остаётся 'unverified' с известным ему паролем), приглашает этот адрес, раскрывает ссылку
        // — и настоящий владелец, подтвердив почту, активирует аккаунт С ПАРОЛЕМ АТАКУЮЩЕГО.
        // Не трогать пароль означало сохранить именно его. Письмо при этом обещает обратное:
        // «пароль не сохранён» (lib/verifyEmail.js).
        //
        // Поэтому на раскрытой ссылке пишется НЕИСПОЛЬЗУЕМЫЙ пароль (`passHash` выше уже такой):
        // после подтверждения им не владеет никто, свой владелец задаёт через «Забыли пароль» —
        // ровно то, что написано в письме. Аккаунт при этом остаётся 'unverified' до письма.
        // Стереть так можно только НЕПОДТВЕРЖДЁННУЮ строку, за которой не стоит доказанный
        // владелец и нет данных; на живой 'active' аккаунт путь не заходит вовсе (409 выше).
        await db.setUserPassword(existing.id, passHash);
        if (!verifyRequired) await db.setUserStatus(existing.id, 'active');
