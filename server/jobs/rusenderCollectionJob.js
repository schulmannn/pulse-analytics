 ...
 * У всех трёх запущенных рассылок аккаунта пришло РОВНО 11 точек независимо от даты отправки
 * (январь и июль) — окно активности у Rusender фиксированное, 11 дней от отправки, а не
 * «сколько накопилось». Хвост открытий за его пределами попадает только в кумулятивные итоги
 * кампании (128/134 и 124/128 открытий), при этом у крошечной рассылки без хвоста (1 открытие)
 * ряд и итоги сходятся точно. Это НЕ повод «дотягивать» ряд до итогов: ряд честно отвечает на
 * вопрос «когда читали», итоги — «сколько всего прочитали». Ровно поэтому витрины держат их
 * РАЗНЫМИ величинами и не складывают.
 */
