...
function createYmCollectionJob({ db, ymFetch, ymCrypto, log }) {
  ...
  // календаря (канон dayOf у МС/IG). Строки upsert здесь и раньше шли только из сгенерированных дней
  // окна, поэтому батч кривая дата не роняла; строгая проверка меняет одно — невозможная дата
  // (2026-02-31) больше не сдвигает начало бэкфилла (раньше Date.parse переносил её в март).
