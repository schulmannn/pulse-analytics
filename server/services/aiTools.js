...
/* ── Read-only инструменты AI-ассистента ──────────────────────────────────────────────────────────
   ...
   Складские инструменты: сводка/клиенты/структура заказов по статусам читают НАШ архив (ms_daily / ms_orders,
...
function createAiTools({ db, sklad }) {
  const definitions = [
    ...
    },
    {
      ...
      description:
        'Распределение заказов МойСклада, созданных за период, по последнему сохранённому статусу: сколько заказов и на какую сумму в каждом статусе ' +
        '(новый, в работе, отгружен, возврат и т.п. — по справочнику организации). Это НЕ исторический статус на конец периода, ' +
        'не переходы между статусами, не конверсия и не последовательность этапов.',
      input_schema: {
        type: 'object',
        properties: {
          channel_id: { type: 'integer', description: 'id источника «МойСклад»' },
  ...
  // ── МойСклад: структура заказов по последнему сохранённому статусу (архив + живой словарь имён) ──
  ...
  // Словарь статусов заказов (id → имя) живым вызовом; null при недоступности — структура
